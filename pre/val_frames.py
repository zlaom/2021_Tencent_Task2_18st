...
# 生成图片存储的位置
datafile_dir = '/home/tione/notebook/dataset/frames/val'
if not os.path.exists(datafile_dir):
    os.makedirs(datafile_dir)

datafile = []
for path, label in tqdm(zip(val_path, val_label)):
    # 生成对应的datafile文件
...
# save to datafile
save_to_file(os.path.join(datafile_save_dir, 'val.txt'), datafile)